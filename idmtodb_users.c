...
int main(int argc, char *argv[])
{
	...
	// psw_expiration_date
        my_bool is_null_psw_expiration_date;
        unsigned long p_psw_expiration_date_length = 0;
        char p_psw_expiration_date[MAX_DATE_LEN];
        ps_insert_user_params[P_IN_PSW_EXPIRATION_DATE].buffer_type = MYSQL_TYPE_STRING;
        ps_insert_user_params[P_IN_PSW_EXPIRATION_DATE].buffer = (char *) p_psw_expiration_date;
        ps_insert_user_params[P_IN_PSW_EXPIRATION_DATE].buffer_length = MAX_DATE_LEN;
        ps_insert_user_params[P_IN_PSW_EXPIRATION_DATE].length = &p_psw_expiration_date_length;
        ps_insert_user_params[P_IN_PSW_EXPIRATION_DATE].is_null = &is_null_psw_expiration_date;

        ps_update_user_params[P_INOUT_PSW_EXPIRATION_DATE].buffer_type = MYSQL_TYPE_STRING;
        ps_update_user_params[P_INOUT_PSW_EXPIRATION_DATE].buffer = (char *) p_psw_expiration_date;
        ps_update_user_params[P_INOUT_PSW_EXPIRATION_DATE].buffer_length = MAX_DATE_LEN;
	...
	char * token = NULL;
		...
		for(token2 = token; *token2 != CSV_SEPARATOR && *token2 != '\0'; ++token2);
                *token2 = '\0';
                #ifdef DEBUG_MODE
                printf("psw_expiration_date is: %s\n", token);
                #endif
	...
	strcpy(mail_buffer, "");
        ...
        if(to_insert || to_update)
        {
				...
				if(pnt_user->psw_expiration_date && strcmp(pnt_user->psw_expiration_date, NULL_IDENTIFIER) && strlen(pnt_user->psw_expiration_date))
                                {
                                        is_null_psw_expiration_date = 0;
                                        #ifdef DEBUG_MODE
                                        printf("psw_exp not null\n");
                                        printf("pnt_user->psw_expiration_date: %s\n", pnt_user->psw_expiration_date);
                                        #endif
                                        strcpy(p_psw_expiration_date, pnt_user->psw_expiration_date);
                                        p_psw_expiration_date_length = strlen(p_psw_expiration_date);
                                }
                                else
                                {
                                        #ifdef DEBUG_MODE
