...
echo 'users_vpn_dict={' > "VPN_Zeus_""$sel_date""_uname_mod.py" && cat "VPN_Zeus_""$sel_date""_uname.csv" | tr ',' ':' | sed ':a;N;$!ba;s/\n/",\n"/g' | sed 's/""//g' | tail -n +2 | sed 's/:/":"/g' | sed '${s/$/"\n}/}' >> "VPN_Zeus_""$sel_date""_uname_mod.py"
