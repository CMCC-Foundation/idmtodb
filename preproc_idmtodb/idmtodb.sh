...
mysql idmdb --raw -h 127.0.0.1 -u root -proot < vpn_query > VPN_expiration_date.csv
...
echo 'users_vpn_dict={' > "VPN_expiration_date_mod_tmp.py" && cat "VPN_expiration_date.csv" | sed "s/   /:/g" | sed ':a;N;$!ba;s/\n/",\n"/g' | sed 's/""//g' | tail -n +2 | sed 's/:/":"/g' >> VPN_expiration_date_mod_tmp.py
